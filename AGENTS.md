...
- Для SCSS:
  ...
  - ЖЕСТКИЙ ЗАПРЕТ: не использовать фиксированные высоты (`height`) для блоков, карточек, секций и контейнеров. Вместо этого использовать `min-height`, внутренние отступы, flex/grid-раскладку и естественную высоту контента. Исключения допустимы только для небольших контролов фиксированного размера (иконки, круглые кнопки, аватары) или по явному запросу.
...
- Растровые изображения при экспорте из Figma сохранять в формате `webp`.
- Если изображение относится к конкретному компоненту, сохранять его в `public/images/<имя_компонента>/`.
...
- Растровые изображения экспортировать в размере, подходящем для экранов с высоким DPI (retina), как минимум в `2x` от отображаемого размера.
